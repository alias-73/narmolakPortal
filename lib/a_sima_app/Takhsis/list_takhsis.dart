...
class pos_registered3_pageState extends State<pos_registered3_page> {
  ...
  Future<List<TakhsisModel>> _getData() async {
    i++;
    if (i < 2) {
      response = await OnlineServices.getTakhsisList({
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: PreferredSize(
          ...
          child: Comp.app_bar("لیست تخصیص", "", Icons.edit, () {})),
      body: Stack(
        children: [
          Column(
            children: [
                    ...
                    onChanged: (txt) {
                      ...
                      if (txt.length > 0) {
                        ...
                        _data.removeWhere((element) =>
                            !element.name2.contains(txt.toString()));
              ...
              Expanded(
                  child: FutureBuilder<List<TakhsisModel>>(
                future: _getData(),
                  ...
                  if (snapshot.hasData) {
                    return ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          return TakhsisAccordion(widget._Agentdata, _data,
                              index, size, _data[index].sanad);
                          AnimationConfiguration.staggeredList(
                            position: index,
                            duration: const Duration(milliseconds: 575),
                            child: SlideAnimation(
                              child: FadeInAnimation(
                                child: TakhsisAccordion(widget._Agentdata,
                                    _data, index, size, _data[index].sanad),
                              ),
                            ),
                          );
                        });
                ...
                Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) => Directionality(
                            ...
                            child: new_takhsis_page(widget._Agentdata))));
              },
            )
          : Center(),
    );
  ...
  Widget item(FaIcon icon, String txt) {
    ...
    return Column(
      children: [
        ...
        Text(txt == null ? "-" : txt,
            style: TextStyle(color: primary, fontSize: 15, letterSpacing: .3)),
      ],
    );
  }
}
