...
class device_listState extends State<device_list> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        key: _scaffoldKey,
        appBar: PreferredSize(preferredSize: Size.fromHeight(Comp.appBarHeight) ,child: Comp.app_bar("لیست دستگاه ها","", Icons.edit, (){})),
                    ...
                    child: TextFormField(
                      onChanged: (txt) {
                        if (txt.length > 0) {
                          ...
                          _data.removeWhere((element) => !element.serial.contains(txt.toUpperCase().toString()));
                          setState(() {});
                        }
                        else {
                          _data.clear();
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              Text(_data[index].status.contains("تخصیص") ? "تخصیص شده" : _data[index].status,
                                  style: TextStyle(
                                      color: primary,
                                      fontSize: 13)),
                            ],
                        ...
                        child: Row(
                          ...
                          children: [
                            FutureBuilder<String>(
                              future: getImg(_data[index].brand,_data[index].model),
                            ...
                            PopupMenuButton(
                              itemBuilder: (context) {
                                ...
                                _data[index].status.contains("آزاد") ? list.add(PopupMenuItem(
                                  child: txt("حذف  "),
                                  value: 2,
                                )) : _data[index].status.contains("رد") ? list.add(PopupMenuItem(
                                  child: txt("حذف  "),
                                  value: 2,
                                )) : const Center();
                              ...
                              child: Column(
                                ...
                                children: [
                                  _data[index].soeich == "پرداخت نوین" ?       Image.asset("assets/images/novin.png", height: 30,) :
                                  _data[index].soeich == "سپهر" ?       Image.asset("assets/images/sepehr.png", height: 30,) :
                                  _data[index].soeich == "سداد" ?       Image.asset("assets/images/sadad.png", height: 30,) :
                                  _data[index].soeich == "به پرداخت" ?  Image.asset("assets/images/behpardakht.png", height: 30,) :
                                  _data[index].soeich == "پاسارگاد" ?   Image.asset("assets/images/pasargad.png", height: 30,) :
                                  _data[index].soeich == "ایران کیش" ?  Image.asset("assets/images/irankish.png", height: 30,) :
                                  const Center(),
                                  const Text("عملیات"), const Icon(Icons.keyboard_arrow_down)],
                              ),
                            )
  ...
  void doo(int value, int index) {
    if      (value == 1) {  terminal  (_data[index].serial);}
    else if (value == 2) { _showDialog(_data[index].serial,_data[index].soeich);}
    else if (value == 3) { _transferDialog(index);}
    else if (value == 4) { getDeviceHistory(_data[index].serial , _data[index].soeich);}
          ...
          builder: (context, setState) {
            return AlertDialog(
              ...
              actions: <Widget>[
                ...
                ElevatedButton(
                    onPressed: () {
                      if (_key1.currentState!.value.isNotEmpty  && _key2.currentState!.value.isNotEmpty) {
                        Navigator.pop(context);
                          transfer(_data2[index].serial, _data2[index].soeich);
                      } else {
  ...
  Future transfer(String serial , String psp) async {
    String response = await (OnlineServices()).transferDevice({"serial":  serial, "agentre": _key1.currentState?.value.toString(), "userre": _key2.currentState?.value.toString() , "agentcode": widget._Agentdata.last.agentCode, "usercode": widget._Agentdata.last.userCode , "psp" : psp});
    String ter;
    response =
  ...
  Future getDeviceHistory(String serial , String psp) async {
    ...
    late Map response;
    response = await OnlineServices.getDeviceHistory({
      "serial": serial,
      "psp": psp
    });
    if (response["data"] != "free") {
   ...
   else _showDialog2( "تاریخچه خالی است");
  ...
  Future terminal(String serial) async {
    ...
    String serialll="";
    if(temp.contains("-")){
      serialll = temp.split("-")[2] + "-" +temp.split("-")[1] + "-" +temp.split("-")[0].replaceAll("\n", "").replaceAll("\t", "").replaceAll("\r", "");}
    else serialll = temp;
      ...
      builder: (BuildContext context) {
        return Container(
          decoration: const BoxDecoration(
              borderRadius: BorderRadius.all(Radius.circular(18))),
          child: AlertDialog(
            title:const Text("آیا از حذف دستگاه اطمینان دارید؟",textAlign: TextAlign.right, style: TextStyle(
            ...
            actions: <Widget>[
              ...
              ElevatedButton(
                  ...
                  child: const Text("  بلی  ", style: TextStyle(
                      fontWeight: FontWeight.bold,
                      color: Colors.white,
                      fontSize: 16),
                  )),
