...
class terState extends State<ter> {
  final _scaffoldKey = GlobalKey<ScaffoldState>();
  int currentPageIndex = 0;
  List<TerminalModel> _data = [];
  List<TerminalModel> _data2 = [];
  int i = 0;
  bool isFree = false;
  late Map response;
  final primary = Color(0xff000000);
  //final secondary = Color(0xfff29a94);
  Future<List<TerminalModel>>_getData() async {
    i++;
    if (i < 2)
    {
      ///  ذک
      response = await OnlineServices.getTerminalList({"agentcode" : widget._Agentdata.last.agentCode, "usercode" : widget._Agentdata.last.userCode} );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: PreferredSize(preferredSize: Size.fromHeight(Comp.appBarHeight) ,child: Comp.app_bar("لیست ترمینال ها","", Icons.edit, (){})),
        ...
        children: [
          Column(
            children: [
              Container(
                  ...
                  child: TextFormField(
                    ...
                    decoration: InputDecoration(
                        ...
                        hintText: "جستجو بر اساس ترمینال یا نام پذیرنده",
              ...
              Expanded(child:
              FutureBuilder<List<TerminalModel>>(future: _getData(),
                builder: (context, AsyncSnapshot<List<TerminalModel>> snapshot) {
                  if (snapshot.hasData) {
                    return ListView.builder(
                        itemCount: _data.length,
                        itemBuilder: (BuildContext context, int index) {
                          return TerminalAccordion(_data,index,size,widget._Agentdata,);
                        });
                  }
