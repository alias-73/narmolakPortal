...
import 'package:sima_portal/a_sima_app/Hesab/EditedReqChangeAccordian.dart';
...
import '../pazirade/editStore.dart';
import 'editHesab.dart';

class editedReqChange_page extends StatefulWidget {
  List<AgentModel> _Agentdata = [];
  editedReqChange_page(this._Agentdata);
  @override
  State<StatefulWidget> createState() => poses_list_pageState();

}

class poses_list_pageState extends State<editedReqChange_page> with SingleTickerProviderStateMixin {
...
//  List<PazirandeModel> _data3 = [];
 ...
 late  Animation<double> _translateButton;
  ...
  Future<List<ChangeModel2>> _getData() async {
    i++;
    if (i < 2) {
      response = await OnlineServices.getChangeHesabList({
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: PreferredSize(preferredSize: Size.fromHeight(Comp.appBarHeight) ,child: Comp.app_bar("لیست درخواست تغییر حساب","", Icons.edit, (){})),
        ...
        children: [
          Column(
            children: [
              ...
              Expanded(child: FutureBuilder<List<ChangeModel2>>(
                future: _getData(),
                builder: (context,
                    AsyncSnapshot<List<ChangeModel2>> snapshot) {
                  if (snapshot.hasData) {
                    return
                      ListView.builder(
  ...
  Widget buildList(BuildContext context, int index) {
    ...
    var blockSize = size.width / 100;
    return
      EditedReqAccordion(_data,index,size,widget._Agentdata)
      // Accordion(
      //     maxOpenSections: 0,
